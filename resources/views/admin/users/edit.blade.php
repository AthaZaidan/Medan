...
@section('title', 'Edit Akun Pengguna — Admin Control')
@section('heading', 'Admin Control — Edit Akun Pengguna')
...
<div class="max-w-2xl space-y-6">
    ...
    <div class="admin-card p-6 space-y-6" x-data="{ role: '{{ old('role', $user->role) }}' }">
        <div class="border-b border-slate-100 pb-4">
            <h2 class="text-base font-bold text-slate-900">Edit Akun: {{ $user->name }}</h2>
            <p class="text-xs text-slate-500 mt-0.5">Ubah informasi akun, peran, wilayah penugasan, atau reset password.</p>
        </div>
        ...
        <form action="{{ route('admin.users.update', $user) }}" method="POST" class="space-y-4">
            ...
            <div>
                <label for="password" class="block text-xs font-bold text-slate-700 mb-1 uppercase tracking-wide">
                    Password Baru <span class="text-slate-400 font-normal capitalize">(Opsional — Biarkan kosong jika tidak ingin mengubah)</span>
                </label>
            ...
            <div x-show="role === 'user_area'" x-transition>
                ...
                <select name="kecamatan_id" id="kecamatan_id"
                    ...
                    @foreach($kecamatans as $k)
                        <option value="{{ $k->id }}" {{ old('kecamatan_id', $user->kecamatan_id) == $k->id ? 'selected' : '' }}>{{ $k->nama }}</option>
                    @endforeach
                </select>
